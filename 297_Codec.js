 ...
 var getPos = function (pos) {
     ...
     i--;
     var temp = Math.pow(2, i) - 1;
     var rest = pos - temp;
     var father = (temp + 1) / 2 - 1 + Math.ceil(rest / 2);
     var left = (temp + 1) * 2 - 1 + rest * 2 - 1;
     var right = left + 1;
     //  var left = (temp + 1) * 2 - 1 + 
     // 此时i为当前pos的层数
     return [father - 1, left - 1, right - 1];
 }
 var solve = function (root, pos_root, list) {
     while (list.length < pos_root + 1) {
         list.push("null");
     }
     list[pos_root] = root.val.toString();
     var pos = getPos(pos_root);
     if (root.left != null) {
 ...
 var deserialize = function (data) {
     ...
     nodeList = [...Array(list.length)].map(_ => null);
     if (list.length == 0) {
         return null;
     }
     nodeList[0] = new TreeNode(list[0]);
